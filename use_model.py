...
flags.DEFINE_float('learning_rate', FOR_OPTIMIZER, 'Initial learning rate.')
flags.DEFINE_string('classlist', 'data_set/classlist.txt', 'File name of each class data')
    ...
    for line in f:
        ...
        if IMAGE_CHANNEL == 1:
            ...
            if cv2.__version__ == '3.0.0':
                img = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY) # opencv3.0
            else:
                img = cv2.cvtColor(img, cv2.cv.CV_BGR2GRAY) # opencv2.4...
...
with open(FLAGS.classlist, 'r') as f: # train.txt
        classlist = []
        for line in f:
            line = line.rstrip()
            l = line.split()
...
for i in range(len(test_image)):
    ...
    if int(r.match(jpgname).group(1))==pred:
        count += 1.0
    count_all += 1.0
print str((count/count_all)*100) + "%"
print "finish"
